...
- **Memory Service** (`memory-service/`) is a FastAPI microservice with pgvector. It calls LiteLLM for embeddings and for LLM-based fact extraction. ⚠️ **Known bug (scheduled for phase-10-1):** embedding code in `embedding.py:59-62` has a hardcoded fallback to `text-embedding-3-small` which always fails and drops to hash-based pseudo-embeddings, making cosine-search meaningless on any embedding miss. Default model names in `docker-compose.yml` are already correct (`EXTRACTION_MODEL=mws/gpt-alpha`, `EMBEDDING_MODEL=mws/bge-m3`). The phase-10 memory work depends on removing this fallback first.
...
**Phase 10 — Persistent conversation memory (planned, 2026-04-11):** Design in `PLAN_db_memory.md`. Adds a `conversation_episodes` table (summary + 1024-dim pgvector embedding + `chat_id`/`message_indices` ref — raw Q/A stays in `openwebui.chat`, not duplicated), two new Memory Service endpoints (`POST /episodes`, `POST /episodes/recall` with optional time window), an `sa_memory_recall` auto-router subagent, `user_id` plumbing from `pipe(__user__=...)` into `SubTask.metadata`, and a `memory_recall` classifier intent with `time_window: {from, to}` parsing. Runs alongside existing fact extraction — facts cover "who the user is / habits", episodes cover "what did we talk about N time ago". Prerequisite: remove the `text-embedding-3-small` fallback in `memory-service/app/embedding.py` (phase-10-1). Task breakdown: `tasks/phase-10-1` .. `phase-10-9`.
